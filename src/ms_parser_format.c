...
/*   Updated: 2021/07/08 12:11:57 by atruphem         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

char	*ms_format_cmd(char *file_name, char **argve)
{
	char	*file_path;
	char	**tab_path;
	int		size_name;
	...
	if (file_name[0] == '/')
		return (ms_format_sl(file_name, argve));
	else if (file_name[0] == '~' && file_name[1] == '/')
		return (ms_format_tile(file_name, argve));
	else if ((file_name[0] == '.' && file_name[1] == '/'))
		return (ms_format_ds(file_name, argve));
	else if ((file_name[0] == '.' && file_name[1] == '.'
			&& file_name[2] == '/'))
		return (ms_format_dd(file_name, argve));
	else
	{	
		tab_path = ms_ext_path(argve);
...
char	*ms_format_file(char *file_name, char **argve)
{
	char	*pwd;
	char	*file_path;
	int		size;
